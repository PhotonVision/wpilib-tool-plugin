...
public class WpilibToolsExtension {
    ...
    public NewTaskSet createExtractionTasks(Action<NewTaskSetConfiguration> configuration) {
        ...
        newConfig.resourceFileName =
                Objects.requireNonNullElse(newConfig.resourceFileName, "ResourceInformation.json");

        final NewTaskSet retSet = new NewTaskSet();
        retSet.extractConfiguration =
                project
                        .getTasks()
                        .register(
                                "extractNativeConfiguration" + newConfig.taskPostfix, ExtractConfiguration.class);
        retSet.extractConfiguration.configure(
                c -> {
        ...
        retSet.fixup =
                project
                        .getTasks()
                        .register("fixupNativeResources" + newConfig.taskPostfix, FixupNativeResources.class);
        retSet.fixup.configure(
                c -> {
        ...
        retSet.hash =
                project
                        .getTasks()
                        .register("hashNativeResources" + newConfig.taskPostfix, HashNativeResources.class);
        retSet.hash.configure(
                c -> {
